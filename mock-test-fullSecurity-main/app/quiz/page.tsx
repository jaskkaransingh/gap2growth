...
import SEBBlockScreen from '@/components/SEBBlockScreen';
...
export default function QuizPage() {
  ...
  return (
    <div className="h-screen overflow-hidden bg-gradient-to-br from-purple-50 via-white to-blue-50 relative flex flex-col">
              ...
              <div className="bg-white rounded-xl sm:rounded-2xl md:rounded-3xl shadow-xl sm:shadow-2xl p-3 sm:p-4 md:p-5 lg:p-6 flex-1 flex flex-col overflow-hidden">
                ...
                <div className="mb-2 sm:mb-3 md:mb-4 flex-shrink-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs sm:text-sm md:text-base lg:text-lg font-bold tabular-nums ${isTimerCritical ? 'text-red-600 animate-pulse' : 'text-blue-600'
                      }`}>
                      {remainingTime}s
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-1.5 md:h-2 overflow-hidden">
                    <motion.div
                      ...
                      transition={{ duration: 0.3 }}
                      className={`h-full rounded-full ${isTimerCritical
                  ...
                  {Object.entries(currentQuestion.options).map(([key, value]) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`w-full p-2 sm:p-2.5 md:p-3 rounded-md sm:rounded-lg md:rounded-xl border sm:border-2 transition-all duration-200 text-left flex items-center justify-between flex-shrink-0 ${isSelected
                      ...
                      >
                        <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
                          {/* Option Letter */}
                          <div
                            className={`w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 lg:w-9 lg:h-9 flex-shrink-0 rounded sm:rounded-md flex items-center justify-center font-semibold text-xs sm:text-sm md:text-base ${isSelected
                        ...
                        {/* Radio indicator */}
                        <div className={`w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0 rounded-full border sm:border-2 flex items-center justify-center ${isSelected ? 'border-blue-500' : 'border-gray-400'
                          }`}>
                          {isSelected && (
                            <motion.div
                              initial={{ scale: 0 }}
        ...
        >
          ...
          {currentQuestionIndex < questions.length - 1 ? (
            <motion.button
              ...
              className={`flex items-center gap-1.5 md:gap-2 px-5 md:px-7 py-2.5 md:py-3 rounded-xl font-semibold transition-all duration-300 text-sm md:text-base shadow-md ${isAutoAdvancing
            ...
            <motion.button
              ...
              whileHover={{ scale: isAutoAdvancing ? 1 : 1.05 }}
              whileTap={{ scale: isAutoAdvancing ? 1 : 0.95 }}
              className={`relative overflow-hidden px-6 md:px-8 py-2.5 md:py-3 rounded-xl font-semibold transition-all duration-300 text-sm md:text-base shadow-lg ${isAutoAdvancing
      ...
      <Watermark careerPath={careerPath} sessionId={sessionId} />
      <CameraFeed isVisible={proctorConfig.enableCamera} />
    </div>
  );
}