...
// Removed SplitText until verified available, using string split if needed
...
const fragmentShader = `
  ...
  void main() {
    ...
    // Correct aspect ratio to prevent shape distortion (circles stretching into ovals)
...
export function ShaderBackground() {
    ...
    return (
        <div ref={canvasRef} className="fixed inset-0 w-full h-full pointer-events-none z-0" aria-hidden>
            <Canvas
                camera={camera}
            ...
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#12131b]/50 via-transparent to-[#12131b]/20" />
        </div>
    );
}
...
export default function Hero({
    ...
    return (
        <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
                ...
                <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
                    {ctaButtons.map((button, index) => (
                        <a
                            key={index}
                            href={button.href}
                            className={`inline-flex items-center justify-center h-9 px-4 rounded-md border text-sm font-medium tracking-[-0.01em] transition-all duration-200 ease-out select-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6366F1]/60 focus-visible:ring-offset-2 focus-visible:ring-offset-[#12131b] ${button.primary
                                ? "bg-gradient-to-br from-[#4F46E5]/40 via-[#6366F1]/30 to-[#06B6D4]/20 backdrop-blur-md text-white border-[#6366F1]/30 shadow-[0_2px_16px_rgba(79,70,229,0.25),inset_0_1px_0_rgba(255,255,255,0.12)] hover:from-[#4F46E5]/60 hover:via-[#6366F1]/50 hover:to-[#06B6D4]/35 hover:border-[#6366F1]/60 hover:shadow-[0_4px_24px_rgba(79,70,229,0.5),0_0_40px_rgba(6,182,212,0.2)] hover:-translate-y-px active:translate-y-0"
                                : "bg-gradient-to-br from-[#4F46E5]/10 to-[#06B6D4]/10 backdrop-blur-sm text-white/75 border-white/10 hover:from-[#4F46E5]/25 hover:to-[#06B6D4]/20 hover:border-[#6366F1]/35 hover:text-white hover:-translate-y-px active:translate-y-0"
                                }`}
                        >
                            {button.text}
                        </a>
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#12131b]/40 to-transparent" />
        </section>
    );
}
