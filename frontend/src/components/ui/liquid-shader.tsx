...
export function InteractiveNebulaShader({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void mainImage(out vec4 O, in vec2 fragCoord) {
        ...
        uv *= 0.5; // Scale UVs down: zooms into the shader, making the entities larger
        vec3 col = vec3(0.0);
        float d = 2.5;
        ...
        for (int i = 0; i <= 5; i++) {
          ...
          // Translating G2G Indigo & Blue, while integrating Dark Blue bases and White highlights
          vec3 base = hasActiveReminders
            ? vec3(0.48, 0.22, 0.92) + vec3(4.0,2.0,5.0)*f   // Focus Purple shift
            : hasUpcomingReminders
            ? vec3(0.38, 0.40, 0.94) + vec3(2.0,5.0,1.0)*f   // Focus Indigo shift
            : vec3(0.05, 0.10, 0.35) + vec3(0.3, 0.4, 0.9)*f + vec3(1.0, 1.0, 1.0)*pow(clamp(f, 0.0, 1.0), 2.0); // Dark Blue base + Indigo + White peaks
      ...
      void main() {
        vec4 outColor;
        mainImage(outColor, vUv * iResolution);
  ...
  return (
    <div
      ref={containerRef}
      className={`fixed inset-0 bg-[#0A0F1F] opacity-60 z-0 pointer-events-none ${className}`}
